...
2. Enter tenant ID (e.g., `my-company`)
3. Enter tenant name (e.g., `My Company`)
4. The GUI will auto-generate `companyInitial` from tenant ID
5. Configure other settings (role, theme, features, etc.)
6. Click **"Save"**
...
```batch
cd apps\member-base\tools\app-manager
manage.bat create-tenant my-company "My Company" --role member
```

**Linux/Mac:**
```bash
cd apps/member-base/tools/app-manager
./manage.sh create-tenant my-company "My Company" --role member
```
