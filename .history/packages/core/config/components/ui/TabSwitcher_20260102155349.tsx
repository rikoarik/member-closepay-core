...
export const TabSwitcher: React.FC<TabSwitcherProps> = React.memo(({
  ...
  if (!animatedValues.current) {
    animatedValues.current = tabs.reduce((acc, tab) => {
      acc[tab.id] = new Animated.Value(activeTab === tab.id ? 2 : 0);
      return acc;
  ...
  // Initial scroll to middle for scrollable variant (only on first render)
  useEffect(() => {
    if (!isSegmented && !hasInitializedScrollRef.current && tabs.length > 0) {
      // Wait for layouts to be measured - check if all tabs are measured
      const checkAndScroll = () => {
        ...
        if (scrollViewRef.current && measuredTabs === tabs.length) {
          ...
          // Retry after a short delay if not all tabs are measured yet
  ...
  useEffect(() => {
    if (scrollViewRef.current && tabLayouts.current[activeTab]) {
      ...
      // Only auto-scroll to activeTab if already initialized (not first render)
      // For segmented variant, always scroll to activeTab
