...
export const HomeScreen = () => {
  ...
  // Calculate default activeTab (tab with order 2)
  const defaultActiveTab = useMemo(() => {
    // After sorting by order, tab with order 2 will be at index 1
    // tabs[0] = order 1, tabs[1] = order 2, tabs[2] = order 3
    if (tabs.length >= 2) {
      return tabs[1].id; // Tab dengan order 2 (home)
    }
  ...
  // Update activeTab when defaultActiveTab changes (only if not manually changed)
  useEffect(() => {
    if (defaultActiveTab && activeTab !== defaultActiveTab) {
      // Only update if current activeTab is not in tabs (meaning tabs changed)
      const currentTabExists = tabs.some(tab => tab.id === activeTab);
      if (!currentTabExists) {
        setActiveTab(defaultActiveTab);
      }
    }
  }, [defaultActiveTab, activeTab, tabs]);
  ...
  // Set posisi awal pager ke tab dengan order 2 (middle tab)
  const hasInitializedRef = useRef(false);
  useEffect(() => {
    if (pagerRef.current && tabs.length > 0 && !hasInitializedRef.current && hasSetInitialTabRef.current) {
      // After sorting by order, tab with order 2 will be at index 1
      // tabs[0] = order 1, tabs[1] = order 2, tabs[2] = order 3
      const middleTabIndex = tabs.length >= 2 ? 1 : 0;
      
      // Scroll pager langsung ke tab dengan order 2
      // activeTab sudah di-set oleh useEffect sebelumnya
      pagerRef.current.scrollTo({
