...
export const HomeScreen = () => {
  ...
  const [activeTab, setActiveTab] = useState<string>(() => {
    // Use function initializer to calculate from tabs
    if (tabs.length >= 2) {
      return tabs[1].id; // Tab dengan order 2 (home)
    }
    return tabs[0]?.id || "home";
  ...
  // Ensure activeTab is set to tab with order 2 when tabs are ready
  useEffect(() => {
    if (tabs.length >= 2 && !hasSetInitialTabRef.current) {
      const order2TabId = tabs[1].id;
      setActiveTab(order2TabId);
      hasSetInitialTabRef.current = true;
    } else if (tabs.length > 0 && !hasSetInitialTabRef.current) {
