...
export const HomeScreen = () => {
  ...
  useEffect(() => {
    ...
    if (tabs.length >= 2) {
      ...
      // Always set to order 2 tab if not already set or if current tab is not order 2
      if (!hasInitializedActiveTabRef.current || activeTab !== order2TabId) {
        setActiveTab(order2TabId);
        hasInitializedActiveTabRef.current = true;
      }
    } else if (tabs.length > 0 && !hasInitializedActiveTabRef.current) {
