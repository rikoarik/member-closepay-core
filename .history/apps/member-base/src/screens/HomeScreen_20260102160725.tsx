...
export const HomeScreen = () => {
  ...
  // Force activeTab to be tab with order 2 when tabs are ready
  useEffect(() => {
    // After sorting by order, tab with order 2 will be at index 1
    // tabs[0] = order 1 (activity), tabs[1] = order 2 (home), tabs[2] = order 3 (news)
    if (tabs.length >= 2) {
      const order2TabId = tabs[1].id; // This should be "home"
      // Always set to order 2 tab, force update immediately
  ...
  // Wait for activeTab to be set first, then scroll
  ...
  useEffect(() => {
    if (pagerRef.current && tabs.length >= 2 && hasSetOrder2TabRef.current && !hasInitializedRef.current) {
      ...
      const middleTabIndex = 1; // Always index 1 for order 2
      
      // Scroll pager langsung ke tab dengan order 2
      // Use setTimeout to ensure activeTab is set first
      setTimeout(() => {
        if (pagerRef.current) {
          pagerRef.current.scrollTo({
